...
F-002 is built and live-tested (`mfactory-smoke` created with labels + protection; awaiting Mattia's manual deletion). F-004 is built: Foreman/builder playbooks in `verbs/`, work-order contract in `templates/`, all shipped into products by the scaffolder; one full local cycle ran with a genuinely fresh builder session and survived independent verification (PR leg pends the first real product). Next ready work: F-005 (adversarial review verb — the merge bar is still CI-green only), F-008 (define verbs). `gstack-main/` is unversioned reference material — loot, don't import (D-001).
