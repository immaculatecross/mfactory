...
| F-002 | backlog | Product scaffolder | One command creates a new product repo with all Layer-1 artifacts, git, GitHub remote, branch protection. |
| F-003 | backlog | Enforcement pack | Pre-commit config, CI workflow templates (all gates from ARCHITECTURE §Layer 3), harness hooks, tripwire library; every failure message states its own fix. |
| F-004 | backlog | Build loop (`build` verb) | Foreman playbook + work-order template + builder playbook; runs one full cycle unattended: pick ready work → branch → build → PR. |
| F-005 | backlog | Adversarial review (`review` verb) | Isolated reviewer produces verdict artifact; wired as required GitHub check; flags, never rewrites. |
...
| F-014 | backlog | Dogfood build #1 | mstack builds and deploys a complete language-learning app with human input limited to WhatsApp approvals. This is the v1 exit test. |
