...
## D-018 · 2026-07-10 · Accepted — Review severity is demonstrated harm, not spec deviation

A finding is BLOCKING only with a demonstrated harm from a closed class list (fail-open, silent wrong result, happy path broken, unreal test, data/secret loss, untruthful artifacts, contract violation), stated on a mandatory `Harm:` line; if the worst demonstrated outcome is a safe stop with a truthful message, it is ADVISORY regardless of the spec letter. On re-review, new BLOCKING findings are limited to repair-introduced issues and harm-class findings (the ratchet rule). **Why:** PRs #9 and #10 showed the adversarial persona plus "any BLOCKING forces request-changes" plus the one-repair limit is structurally fatal — every re-review of a nontrivial surface finds new defensible blockers, so PRs die serially while every found outcome was fail-safe; forced harm articulation is the calibration (a reviewer who must write "Harm: the loop stops safely" has reclassified their own finding). **Consequences:** the verdict format and both rules live in `verbs/review.md`; the repair limit and the binary any-BLOCKING-blocks rule stay unchanged — the fix is what qualifies, not the consequence.

## D-019 · 2026-07-10 · Accepted — Adjudication: bounded override authority above a repeated rejection

When the one repair is spent and re-review still requests changes, the Foreman dispatches a fresh adjudication session (`verbs/review.md` §Adjudication) that rules on each standing BLOCKING finding against D-018: all overruled → it records the override in `LOG.md` on the branch and issues a genuine approve verdict at the new head; any sustained → one final repair scoped to the sustained findings, then a fresh review; still failing → `blocked` plus an escalation report to Mattia, the last resort. **Why:** without an appeal path the review gate holds unaccountable veto power over the factory's throughput, but the overriding party must not be the Foreman (who wants throughput) nor able to bury the act (the LOG commit makes every override history). **Consequences:** adjudication never overrules a sustained harm-class finding; the audit's one-verdict-per-head invariant is untouched because the override rides a new commit; escalation to the human is the ladder's floor, not its default.
