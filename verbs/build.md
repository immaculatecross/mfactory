...
5. **Judge the result** by the builder's exit report and the facts (branch, PR, checks). Verify with the committed gate commands and tests — never improvised shell (D-017).
   ...
   - CI failed, or review requested changes → dispatch **one** repair session: same work order with the failure output or findings appended. New commits void the old review — re-dispatch it. Never repair endlessly.
   - Re-review still requests changes after the one repair → dispatch a fresh **adjudication** session (`verbs/review.md` §Adjudication). It rules on each standing BLOCKING finding against the materiality bar (D-018): all overruled → it issues the approve verdict and you merge; any sustained → dispatch **one final repair scoped strictly to the sustained findings**, then a fresh review. If that still fails, escalate: mark the feature `blocked` in the next PR, and report to the Owner/Mattia with the sustained findings, what was tried, and your recommendation — the last resort, never the reflex.
