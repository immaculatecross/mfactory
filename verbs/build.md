...
5. **Judge the result** by the builder's exit report and the facts (branch, PR, checks). Verify with the committed gate commands and tests — never improvised shell (D-017).
   - `done` and CI is green → dispatch a fresh **review** session (`.mfactory/verbs/review.md`) on the PR. Merge (squash) only when CI is green, the `review` status is success, **and** the consistency audit passes (`.mfactory/ci/mfactory-audit-review <owner/repo> <n>` — deterministic check that the status is backed by a verdict comment on the same SHA). If the reviewer requests changes, treat its BLOCKING findings as a repair work order.
