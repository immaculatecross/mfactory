...
PR #9's first review found invalid sentinel arrangements, log collisions, missing remediation, and tests blind to lost stderr; its second full-head review then found concurrent loops could dispatch duplicate Foremen. The repair limit closed #9. This fresh work order consolidates every finding as an invariant: an atomic repo lock with active/stale diagnosis and signal cleanup; collision-proof full logs; strict non-whitespace final sentinels; a quoting-safe harness path; target-qualified brake recovery; and mutation-sensitive failure assertions. `verbs/build.md` step 8 ends each fresh Foreman cycle with the sentinel instead of looping internally; all driver runtime state is gitignored.
