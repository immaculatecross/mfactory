...
## 2026-07-10 — Audit hardened same-day: first-line-only matching (the PR #6 review finding)

PR #6's isolated reviewer approved but demonstrated an advisory bypass: the audit matched a verdict line anywhere in a comment, so an ordinary comment narrating or quoting the exact line ("Do NOT merge… the verdict would be: VERDICT: approve SHA=…") passed as genuine. Encoded per D-017 the same day: the audit now reads only the **first line of each comment**, exactly what `verbs/review.md` already mandated, so a verdict anywhere else in a body is structurally invisible. Also fixed: carriage returns are stripped, so a genuine verdict typed in the GitHub web UI (CRLF) no longer fails with a misleading message.

This PR's own first review then caught the deeper flaw (BLOCKING): extraction lived in a jq filter the tests never exercised — the bypass "regression test" could not fail, and reverting the filter left all tests green. Repaired in the permitted attempt: comment bodies now travel base64-encoded (one comment per line, boundaries intact) and first-line extraction happens inside the script, in the tested unit. The suite (18 cases) feeds base64 bodies exactly as the fetch path delivers them; mutation-checked — regressing extraction to whole-body matching turns the bypass case red. A failed comments fetch now dies with its true cause instead of "no verdict found" (the review's advisory). The fetch path re-validated PR #6's real verdict live.

---

## 2026-07-10 — Standalone architecture explainer records shipped reality

Added `docs/architecture-now.html`, an offline, responsive, print-ready briefing of the intended architecture, the F-001–F-005 machinery actually shipped, the manual substitutions in the Mora dogfood, and the difference between useful gate latency and avoidable orchestration friction. Repository files, mfactory PR #6, Mora's learnings, and Mora PR #1 provide the evidence trail.

Verification: HTML passed `tidy -errors -quiet` and `xmllint --html --noout`; `git diff --check` and tripwires passed; responsive layouts at 375px and 1440px plus print rendering were inspected. The work exposed stale `STATE.md` text claiming PR #4's review hardening was absent after fresh PR #6 had landed it. Per D-017, that drift lesson is encoded in `verbs/build.md` as a mandatory post-merge STATE-versus-master truth check that makes artifact repair the next work unit; this PR also corrects `STATE.md`.

---

## 2026-07-10 — The loop driver: `bin/mfactory-loop` makes `build` actually autonomous

Until now nothing re-invoked the Foreman — "autonomous" meant Mattia running each cycle by hand. `bin/mfactory-loop` is the missing spine: it checks the emergency brake (`.mfactory/STOP`, exit 2 — D-014's kill, distinct from CONTROL steering), dispatches one fresh Foreman session per cycle (D-003), logs each cycle to `.mfactory/loop/`, and treats only the report's final line as protocol — exact `NEXT: continue` or `NEXT: stop <non-whitespace reason>`. Any other final line, agent crash, or cycle cap (default 8, a runaway leash) stops fail-safe with its own fix. `$MFACTORY_AGENT_CMD` is a quoting-safe harness executable path; launcher scripts must `exec` their agent so the owned PID remains the process the loop tears down.

PR #9's reviews found invalid sentinels, log collisions, blind tests, and duplicate concurrent Foremen; its repair limit closed the PR. Fresh PR #10 consolidated those invariants, then reviews exposed a non-racing lock test, TERM-resistant teardown, an orphan launch window, and unproved prompt/brake/remediation claims. Mattia explicitly authorized finishing #10 instead of repeating fresh-PR churn. The repair simplifies the contract—only the final report line is protocol and launchers transfer PID ownership with `exec`—defers signals across child launch, bounds TERM→KILL teardown, races lock acquisition, verifies one complete prompt and between-cycle braking, and makes `die` reject empty fixes centrally. Full logs, target-qualified recovery, and step 8's fresh-cycle sentinel remain enforced.

---

## 2026-07-10 — Review calibration: severity is demonstrated harm; adjudication above a repeated rejection

PRs #9 and #10 exposed a structural failure, not a quality signal: the adversarial reviewer ("an approve means *you* couldn't break it") plus "any BLOCKING forces request-changes" plus the one-repair limit kills any PR with a nontrivial surface, because every re-review of concurrency/signal/parsing code finds new defensible blockers. The tell: PR #10's second review raised five blockers, nearly all present in code the first review had already examined — and every outcome found across both PRs was fail-safe (the loop stops with a truthful message). Severity was measuring cleverness of attack, not cost of outcome.

Encoded per D-017, the same day, in two new decisions. **D-018** (in `verbs/review.md` §Findings): BLOCKING now requires a demonstrated harm from a closed class list — fail-open, silent wrong result, happy path broken, unreal test, data/secret loss, untruthful artifacts, contract violation — stated on a mandatory `Harm:` line; a safe stop with a truthful message is ADVISORY regardless of spec letter, and re-reviews can only block on repair-introduced or harm-class findings (the ratchet rule). The forced articulation is the calibration: a reviewer who must write "Harm: the loop stops safely" has just reclassified their own finding. **D-019** (in `verbs/review.md` §Adjudication and `verbs/build.md` step 5): when the one repair is spent and re-review still rejects, the Foreman dispatches a fresh adjudication session — not the Foreman itself, which wants throughput — that sustains or overrules each standing blocker against the bar. All overruled → the override is committed to `LOG.md` on the branch and a genuine approve verdict lands on the new head (the audit's one-verdict-per-head invariant untouched); any sustained → one final repair scoped to the sustained findings; still failing → `blocked` plus an escalation report to Mattia, the last resort. The repair limit and the binary any-BLOCKING-blocks rule are deliberately unchanged — the fix is what qualifies as BLOCKING, not the consequence. Aligned with the industry bar (Google's review standard: approve what definitely improves the system; label non-mandatory findings) and the CVSS habit of deriving severity from articulated impact.
