...
PR #9's reviews found invalid sentinels, log collisions, blind tests, and duplicate concurrent Foremen; its repair limit closed the PR. Fresh PR #10 consolidated those invariants, whose first review then broke compound-sentinel validation, resistant-agent teardown, and a concurrency test that did not actually race acquisition. Its one repair uses the same strict matcher for the final line, races two gated processes, and bounds signal cleanup with TERM then KILL; collision-proof full logs, quoting-safe harness paths, target-qualified brake recovery, and mutation-sensitive failure assertions remain covered. `verbs/build.md` step 8 ends each fresh Foreman cycle with the sentinel; all runtime state is gitignored.
