...
PR #6's isolated reviewer approved but demonstrated an advisory bypass: the audit matched a verdict line anywhere in a comment, so an ordinary comment narrating or quoting the exact line ("Do NOT merge… the verdict would be: VERDICT: approve SHA=…") passed as genuine. Encoded per D-017 the same day: the audit now reads only the **first line of each comment** (`--jq '.[].body | split("\n")[0]'`), exactly what `verbs/review.md` already mandated, so a verdict anywhere else in a body is structurally invisible. Also fixed the reviewer's second advisory: carriage returns are stripped, so a genuine verdict typed in the GitHub web UI (CRLF) no longer fails with a misleading "no verdict found". Test suite now 16 cases, including the demonstrated bypass and the CRLF path; the hardened fetch path re-validated PR #6's real verdict live.
