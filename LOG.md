...
Until now nothing re-invoked the Foreman — "autonomous" meant Mattia running each cycle by hand. `bin/mfactory-loop` is the missing spine: it checks the emergency brake (`.mfactory/STOP`, exit 2 — D-014's kill, distinct from CONTROL steering), dispatches one fresh Foreman session per cycle (D-003), logs each cycle to `.mfactory/loop/`, and obeys the report's one exact final sentinel — `NEXT: continue` or `NEXT: stop <reason>`. Missing, bare, conflicting, non-final, or lookalike sentinels, agent crashes, and the cycle cap (default 8, a runaway leash) all stop fail-safe with messages that state their fix. The harness is pluggable via `$MFACTORY_AGENT_CMD` (default `claude -p`), which also makes the driver testable end to end with a fake agent.

The first adversarial review found four BLOCKING gaps: invalid sentinel arrangements passed, rapid invocations could overwrite logs, missing option values had no remediation, and tests could not detect lost stderr or removed `Fix:` text. The one permitted repair uses collision-proof log names and enforces exactly one valid sentinel as the report's final line. The suite now attacks each demonstrated sentinel case, fixes time deterministically to prove logs cannot collide, verifies both stdout and stderr survive in normal and crash logs, and asserts remediation text on every exercised failure path. `verbs/build.md` step 8 now ends the cycle with the sentinel instead of looping internally — the old internal loop quietly contradicted D-003's fresh-Foreman-per-cycle; both gitignores exclude the driver's runtime state from history.
